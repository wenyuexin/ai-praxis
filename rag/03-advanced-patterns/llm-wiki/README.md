...
因此它放在 "高级范式" 下是合适的：它是一种完整的范式反转，而不只是某种检索增强。它暂列于 `rag/` 下，与狭义 RAG 共享"外部知识如何被组织并供给 AI"这一相邻问题带，但不是 RAG 的子集。
...
- 与本目录对象直接相关的支撑组件，可在比较或关系说明中被引用；但像 `PageIndex` 这类可独立成立的检索框架，长期主条目应放回更稳定的检索骨架，而不是留在本目录对象层
