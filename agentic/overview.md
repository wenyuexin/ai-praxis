...
人机交互层不只是 UI 或聊天界面，而是人类如何委托、约束、观察、接管和信任 Agent 系统。在 coding-agent / vibe coding 这类长任务场景中，人类还可能通过任务边界、上下文状态、权限控制、review、validation 与接管机制，为 Agent 提供外部支架；`04-human-agent-interaction/human-in-the-loop/vibe-coding-human-harness.md` 可作为这一问题的案例型入口，但不应被外推为通用理论。
...
- `04-human-agent-interaction/` 中 human-in-the-loop、delegation granularity、trust calibration 的正文专题；其中 vibe coding human harness 已有案例型入口，但仍缺跨场景抽象。
...
- Sources: `agentic/README.md`、`01-foundations/overview.md`、`02-single-agent/overview.md`、`03-multi-agent/overview.md`、`04-human-agent-interaction/overview.md`、`05-environments/overview.md`、`06-frameworks-and-tools/overview.md`、`07-evaluation/overview.md`、`agentic/backlog.md`
- Trace: 从顶层 backlog 中“Agentic 顶层 overview 缺口”出发，先基于已有主干文档形成早期综述；本次还吸收了一轮临时反馈中较稳的问题意识，但不回填具体对象判断、契约草案或未经核验的横向结论；主题缺失和待补证方向继续保留在 `agentic/backlog.md` 与各子目录 backlog 中
