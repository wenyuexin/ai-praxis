...
用户不是按任务步骤授权，而是按能力边界授权，例如允许哪些工具、路径、预算、网络范围与副作用。
...
用户把一整条固定工作流交给 Agent 执行，Agent 的自主性主要体现在局部参数、分支选择或恢复处理，而不是重写流程本身。
...
因此比较 delegation 时，不能只说“更自动”或“更保守”，而要看它把自主权放在哪一层。
...
- 与 `../agent-ui/`：不同委托粒度要求不同的任务可见性、计划呈现与中断操作面。
...
- delegation granularity 是否应进一步拆成委托对象、权限厚度、纠偏机制三个独立专题
