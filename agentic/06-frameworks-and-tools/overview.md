...
`06-frameworks-and-tools` 必须坚持工程实现视角。边界规则如下：

| 问题 | 主归属 | `06` 中如何出现 |
|------|--------|------------------|
| planning / memory / tool-use / reflection 的理论和设计原则 | `../02-single-agent/` | 作为具体框架实现章节或对比维度 |
| multi-agent collaboration / coordination 的模式分类 | `../03-multi-agent/` | 分析 AutoGen、CrewAI、MAF 等如何落地这些模式 |
| sandbox、browser、code execution environment 的环境设计原理 | `../05-environments/` | 分析具体工具或系统如何接入执行环境 |
| benchmark、metrics、evaluation methodology | `../07-evaluation/` | 分析具体产品或框架在评测中的表现和工具支持 |
| framework / tool / product / project 的架构与生态 | `./` | 本目录主线 |

判断规则：

- 回答“这个能力/模式/原理是什么”时，放回 `02/03/05/07`。
- 回答“这个框架/工具/项目具体怎么实现、怎么集成、怎么选型”时，放在 `06`。
- 横跨多层的内容采用主归属 + 交叉引用，不在多个目录重复维护。
...
- 成熟框架、工具、项目对象进入 `01-frameworks/`、`02-coding-tools/`、`03-project-studies/` 或 `04-skill-and-tool-systems/`。
- 对象之间的横向比较进入 `05-comparisons/`。
- 前沿但尚不稳定的对象进入 `backlog.md`，不直接创建主干目录。
- 底层能力主题进入 `02/03/05/07` 的相应目录，`06` 只保留工程实现引用。
- 证据不足或来源不稳定的结论只进入 backlog 或 conflict，不进入 overview 作为定论。
...
1. 补全 `01-frameworks/` 中 LangGraph、AutoGen、CrewAI 等代表性框架的对象资料。
2. 完善 `02-coding-tools/` 中 Claude Code、Codex、OpenHands 等编码 Agent 工具资料。
3. 继续扩展 `03-project-studies/` 中 Hermes Agent、OpenClaw 等完整系统案例。
4. 将 Skill、Tool、插件、MCP 集成等工程实现沉淀到 `04-skill-and-tool-systems/`。
5. 在 `05-comparisons/` 中建立框架选型、编码工具对比和生态地图。
6. 使用 `backlog.md` 跟踪前沿对象、候选项目和证据不足项。
