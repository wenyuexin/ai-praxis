...
- Sources:
  ...
  - `Claude Skills` 第三方高信号解读 — https://www.kdnuggets.com/anthropics-complete-guide-to-claude-skills-building
- Trace: 这些候选对象最初来自外部调研输入，随后经过一轮筛选，没有直接升格为正文定论，而是先按 `05-comparisons/` 下正在形成的两条主线做对象队列整理：一组服务纯文本 skill 治理，一组作为执行约束边界样本。当前已将能明确恢复到真实论文或公开文档的对象改写回真实来源；仍偏二手或边界样本的对象继续保留候选身份，等待更稳的一手材料后再决定是否升级。
- Needs:
  - 为核心白名单对象补一手论文 / 官方文档核验
