...
- `updated_channels`：该 checkpoint 中被更新的通道。
- `metadata`：checkpoint 来源、step、parent checkpoint 等元信息。
- `pending_writes`：与 checkpoint tuple 关联的中间 writes。

其中 `next` / `tasks` 更适合写成文档或 debug / runtime view 中暴露的执行视图，而不是直接并入当前 OSS `Checkpoint` TypedDict 的核心 schema。这说明 checkpoint 保存的是 graph runtime 的结构化状态，而不是文件系统、容器或进程级快照。
...
本轮源码还观察到 `copy_thread`、`prune`、`delete_for_runs` 与 DeltaChannel 相关 beta 接口；这些说明 saver 生命周期能力正在扩展，但不应把它们提前写成所有 backend、所有部署形态的稳定生产语义。
...
- `update_state` / state edit 的持久化语义需要分层表述：当前 OSS runtime 中已经能观察到普通 state edit 写入 `source=update` checkpoint，显式 `as_node="__copy__"` / time-travel copy 路径写入 `source=fork` checkpoint；Python SDK / server runtime 类型说明显示 platform 表面通过 thread state endpoint 暴露 state update，而不是单独暴露名为 `branch` 的对象。

但这里仍要保持 stop-line：TTL 到期是否自动触发 `delete_thread`、不同 saver 的清理策略是否一致、server 私有实现如何处理 interrupted thread 的 outstanding interrupt，以及 platform 文档中的 branch 叙述是否还有额外包装，当前都还不适合写成统一定论。
...
- 当前 OSS checkpoint schema 围绕 `channel_values`、`channel_versions`、`versions_seen`、`updated_channels` 组织，checkpoint tuple 还携带 metadata 与 pending writes。
- 文档或 debug / runtime view 中的 `next` / `tasks` 也属于 graph execution view，而不是 OS / process / container / filesystem 层。
...
- server 私有实现如何处理 interrupted thread 上的 `update_state`、outstanding interrupt 与平台 UI / branch 叙述之间的关系。
