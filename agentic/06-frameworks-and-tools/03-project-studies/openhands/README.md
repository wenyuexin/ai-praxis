...
- **阶段二：源码研究**：`openhands/app_server/` 层源码核验已完成，重点结论已写入运行时与沙箱专题；SDK 层 workspace 绑定关系、agent-server 入口与跨层恢复链路仍待继续补证。
...
├── README.md
├── official-docs.md
└── runtime-and-sandbox.md
```

## 阅读入口

- `official-docs.md`：OpenHands 官方资料阶段调研整理，包含资料清单、可用结论、风险点和源码阶段核验清单。
- `runtime-and-sandbox.md`：基于 `https://github.com/OpenHands/OpenHands` 中 `openhands/app_server/` 的源码核验结果，集中记录 SandboxService、持久化路径、卷挂载与当前证据边界。
