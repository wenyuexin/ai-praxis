...
如果只从当前已整理的官方文档与对象内 intake 看，OpenHands 在 vibe coding 视角下最稳定支持的是 `acceptance / constraint / correction` 三类 human harness：`confirmation policy` 提供不同人工审批强度，`hooks` 提供结构化约束层，CLI `Esc` 暂停提供纠偏入口。相比 Claude Code 以模式切换调节控制强度，OpenHands 更像通过 policy / hook / control-plane 提供 system-level 控制层。

这里仍要保持 stop-line：这些能力更接近系统配置、运行控制或企业管理层机制，不能直接外推成“OpenHands 已经把 goal harness 做成显式一等交互面”，也不能据此写成“用户一定会低摩擦持续补 harness”。
