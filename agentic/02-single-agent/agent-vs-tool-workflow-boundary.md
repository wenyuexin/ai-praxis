...
- `reasoning-and-acting-loop/` 中的循环结构是不是 agent 的必要成分
...
所以它天然横跨 `reasoning-and-acting-loop/`、`tool-use/`、`planning/`、`memory/`。
...
- 与 `reasoning-and-acting-loop/README.md`：持续 reasoning-action-observation loop 是区分 agent 与普通 workflow 的关键结构之一。
