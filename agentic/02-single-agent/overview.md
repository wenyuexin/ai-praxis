...
- `planning/` 决定步骤结构
- `reasoning-and-acting/` 决定局部循环
- `tool-use/` 决定外部能力边界
- `memory/` 决定跨时间状态保持
- `self-reflection/` 决定修正与迭代能力
- `patterns/` 负责总结这些能力如何被组合

---

## 六、与其他目录的关系

- 与 `../01-foundations/`：继承 agent 定义、认知架构和系统模型
- 与 `../03-multi-agent/`：单智能体能力是多智能体分工的基础
- 与 `../05-environments/`：环境约束决定工具执行、状态保持和反馈获取方式
- 与 `../06-frameworks-and-tools/`：框架和产品是这些能力的实现载体，不应反过来替代能力层分类
- 与 `../07-evaluation/`：单智能体能力必须通过任务完成度、可靠性和成本来验证
