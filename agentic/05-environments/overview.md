...
- 与 `../02-single-agent/`：环境直接影响工具执行、状态保持和反馈获取方式
- 与 `../03-multi-agent/`：通信成本、共享状态、subtask 隔离都依赖环境能力
- 与 `../06-frameworks-and-tools/`：框架和工具只是在不同环境假设上落地的实现案例
- 与 `../07-evaluation/`：环境定义了哪些 benchmark 可信、哪些结果可复现
