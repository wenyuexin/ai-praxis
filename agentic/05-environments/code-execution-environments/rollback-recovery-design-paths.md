...
LangGraph 则提供了另一种对照：它已经能稳定支持 replay、fork、`update_state`、interrupt / resume 和 durable execution，但其 replay / time-travel 语义仍然围绕 graph state checkpoint 与 workflow control flow 展开，而不是默认重建 workspace 文件系统或 execution environment。也就是说，结构化 replay 的存在只能先证明“某层状态可被重放 / 分支 / 继续执行”，不能自动证明“系统所有相关状态都能被重建”。

SWE-agent 又从另一侧补充了一个很有价值的对照：其 `trajectory` 可以记录 action / observation / state，`run-replay` 可以把这些 actions 在环境中重新执行；但 trajectory 中默认的 `state` 主要覆盖 `open_file` 与 `working_dir`，不等于完整文件系统快照。这意味着 action replay 的存在可以证明“系统能够基于历史动作重新获得近似可用状态”，但不足以证明“当前 trajectory 结构已经足够独立重建完整 workspace 文件系统”。

这提醒我们：replay-based recovery 必须明确说明自己重放的是哪一层状态。
...
当前跨案例对照已经能提供一个更具体的恢复锚点分类：

- **OpenHands** 更偏 `conversation_id` / `sandbox_id` 这类会话与执行实体锚点；它优先回答“是否还能连回同一会话 / sandbox 身份并继续执行”。
- **SWE-agent** 更偏 `base_commit` / trajectory 这类 repo baseline 与 action history 锚点；它优先回答“是否还能把代码库退回已知基线并重放关键动作”。
- **LangGraph** 更偏 `thread_id` / `checkpoint_id` 这类 graph runtime state 锚点；它优先回答“是否还能从某个 workflow checkpoint 恢复 graph 执行”。

这组对照提醒我们：不同系统都可能使用 resume / replay / restore 这些词，但它们锚定的状态层并不相同。如果不先回答这一层，技术路径比较会很容易失焦。
...
- **OpenHands** 的 `pause` / `interrupt` / `resume` 更强调 conversation / sandbox 连续性与可重新附着。
- **SWE-agent** 的 `git reset --hard`、trajectory 与 `run-replay` 更强调回到 repo baseline 后重新获得可执行状态。
- **LangGraph** 的 interrupt / `Command(resume=...)` / time travel 更强调 graph control flow 与 workflow state continuation。
