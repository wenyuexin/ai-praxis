...
- Sources: `agentic/temp/web-search/4.md`, `workspace-structure.md`, `workspace-checkpoint.md`, `workspace-traceability.md`, `rollback-recovery-design-paths.md`, `traceability-object-model.md`, `../../06-frameworks-and-tools/03-project-studies/openhands/runtime-and-sandbox.md`, `../../06-frameworks-and-tools/03-project-studies/openhands/scheduling-performance.md`
- Trace: 先从 `web-search/4.md` 中关于 workspace 生命周期绑定、共享基线、overlay / copy-on-write、git worktree 与 per-task isolation 的材料回流，结合现有 workspace / checkpoint / traceability 专题整理成主干专题；随后再用 OpenHands 与 `software-agent-sdk` 的交叉源码核验结果，补强“恢复能力可分层存在，但不自动等于文件系统重建”的边界，并用 OpenHands 调度性能风险图补充启动 / 恢复成本的编排侧约束。
