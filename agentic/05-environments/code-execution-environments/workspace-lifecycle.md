...
从当前已核验的 OpenHands / `software-agent-sdk` 交叉证据看，这种“部分解耦”至少可以进一步拆成三层：

- app / conversation 层保留 `conversation_id`、`sandbox_id`、`conversation_url`、`session_api_key` 等可恢复连接信息
- workspace backend / agent-server 层显式提供 `pause()` / `resume()`、conversation restore、持久化 conversation 扫描恢复等能力
- 底层 runtime / container / remote session 层再决定恢复时究竟是沿用原有执行实体，还是只沿用其逻辑身份与连接元数据

这也说明“支持恢复”并不自动等于“文件系统状态已经被完整重建”；很多系统首先恢复的是可寻址会话、连接入口和运行时身份，然后才谈工作目录与文件状态连续性。
...
当前 OpenHands / `software-agent-sdk` 交叉源码更支持一种中间表述：系统可以在“conversation 恢复能力”上明显强于“文件系统重建能力”。也就是说，用户重新连回同一会话、同一 sandbox 身份或同一 agent-server conversation，并不自动证明底层 workspace 文件状态一定可由 event replay 或持久化 state 独立重建；现有证据更偏向“先恢复连接与执行身份，再尽量沿用既有 runtime / volume / working_dir 连续性”。
...
需要注意的是，真实系统里“per-task isolated”未必意味着每次都从零创建一套完全独立文件系统。有些实现会把会话级 identity、sandbox identity 或 agent-server conversation 作为恢复锚点，再在该锚点之下维持近似 per-task 的执行边界。换句话说，隔离模型和恢复模型经常交叉存在，而不是二选一。
...
OpenHands 的调度风险图提供了一个更具体的提醒：启动成本不只是“容器启动慢”，还包括 sandbox 创建 / resume 的等待轮询、agent-server 健康检查、workspace backend HTTP bridge、setup script / git clone 等串行步骤，以及 event persistence 在恢复或检索时的读取成本。也就是说，workspace lifecycle 设计越依赖长链路编排，越需要显式管理尾延迟、并发上限、清理失败和可观测性。

如果系统还支持 `pause`、`interrupt`、`resume` 等恢复动作，这个权衡会进一步复杂化：

- `pause` 更像保守地等待当前动作自然收束，再保留执行上下文
- `interrupt` 更像立即切断当前执行，再尝试把会话停在一个可恢复点
- `resume` 则可能只恢复连接和运行时身份，也可能连带恢复既有工作目录状态

因此，恢复动作的粒度本身就是 lifecycle 设计的一部分，而不是单纯附属于“已有 workspace 是否复用”的小机制。
...
反过来，workspace 越强调跨 runtime / sandbox 的可恢复连续性，系统就越容易引入额外调度面：连接身份、健康检查、远端 workspace bridge、event store 与清理回收都可能成为性能瓶颈。OpenHands 的源码观察更支持这类中间结论：恢复能力和调度成本应一起评估，而不是只比较“是否能 resume”。
...
- Status: Inferred / Observed / Verified
- Sources: `workspace-structure.md`, `workspace-checkpoint.md`, `workspace-traceability.md`, `rollback-recovery-design-paths.md`, `traceability-object-model.md`, `../../06-frameworks-and-tools/03-project-studies/openhands/runtime-and-sandbox.md`, `../../06-frameworks-and-tools/03-project-studies/openhands/scheduling-performance.md`, OpenAI `Agents SDK` 更新说明 https://openai.com/index/the-next-evolution-of-the-agents-sdk, Temporal `Introducing Temporal and agentic sandboxes: the OpenAI Agents SDK` https://temporal.io/blog/introducing-temporal-and-agentic-sandboxes-openai-agents-sdk, NVIDIA `Secure Agent Workspace Reference Design` https://docs.nvidia.com/enterprise-reference-architectures/secure-agent-workspace-reference-design/latest/abstract.html, LangGraph checkpointers https://docs.langchain.com/oss/python/langgraph/checkpointers, LangGraph persistence https://docs.langchain.com/oss/python/langgraph/persistence, LangGraph `Delta Channels` https://www.langchain.com/blog/delta-channels-evolving-agent-runtime, Turso AgentFS repo https://github.com/tursodatabase/agentfs, Turso AgentFS docs https://docs.turso.tech/agentfs/introduction, IBM `chuk-artifacts` https://github.com/IBM/chuk-artifacts, IBM `chuk-mcp-runtime` https://github.com/IBM/chuk-mcp-runtime, GitHub self-hosted runners reference https://docs.github.com/en/actions/reference/runners/self-hosted-runners, Development Container Specification https://devcontainers.github.io/implementors/spec, PROV-AGENT arXiv https://arxiv.org/abs/2508.02866, PROV-AGENT IEEE metadata https://impact.ornl.gov/en/publications/prov-agent-unified-provenance-for-tracking-ai-agent-interactions-, MAIF arXiv https://arxiv.org/abs/2511.15097, Microsoft Foundry Build 2026 runtime post https://devblogs.microsoft.com/foundry/agent-service-build2026, Microsoft Foundry April 2026 tracing update https://devblogs.microsoft.com/foundry/whats-new-in-microsoft-foundry-apr-2026, AGNTCY Agent Directory blog https://outshift.cisco.com/blog/ai-ml/agntcy-agent-directory-find-and-publish-ai-agents
- Trace: 先从一轮围绕 workspace 生命周期绑定、共享基线、overlay / copy-on-write、git worktree 与 per-task isolation 的外部调研中抽取较稳线索，再把能恢复到真实对象和公开来源的部分改写回这些对象本身，结合现有 workspace / checkpoint / traceability 专题整理成主干专题；随后再用 OpenHands 与 `software-agent-sdk` 的交叉源码核验结果，补强“恢复能力可分层存在，但不自动等于文件系统重建”的边界，并用 OpenHands 调度性能风险图补充启动 / 恢复成本的编排侧约束。
- Needs: 继续补充官方文档、开源实现源码和跨产品对照，特别是多 agent 共享边界、overlay 并发语义、remote runtime 与 worktree 成本比较，以及恢复连接能力与文件系统连续性之间的真实耦合强度。

关键 Claim：

| Claim | Status | Sources | Notes |
|---|---|---|---|
| Workspace 不应被等同为 sandbox 内路径，而应作为 task-specific working context 理解。 | Inferred | `workspace-structure.md`; OpenAI `Agents SDK` 更新说明; Temporal `Introducing Temporal and agentic sandboxes: the OpenAI Agents SDK`; NVIDIA `Secure Agent Workspace Reference Design` | 已进入主干口径，但仍需更多产品定义对照。 |
| Workspace 生命周期可以与 runtime / sandbox 强绑定，也可以通过持久化状态、checkpoint、overlay 或 worktree 部分解耦。 | Observed / Inferred | LangGraph checkpointers / persistence / `Delta Channels`; `workspace-checkpoint.md`; `rollback-recovery-design-paths.md`; GitHub self-hosted runners reference; Development Container Specification | 具体系统的绑定程度仍需逐一核验。 |
| OpenHands / `software-agent-sdk` 交叉源码表明：conversation 恢复能力、workspace backend 的 pause/resume 能力、agent-server 的持久化恢复能力可以分层存在，但这不自动证明文件系统状态可被独立重建。 | Observed | `../../06-frameworks-and-tools/03-project-studies/openhands/runtime-and-sandbox.md` | 当前更像“先恢复连接与执行身份，再尽量沿用既有 runtime / volume / working_dir 连续性”。 |
| Per-task isolation、shared baseline + isolated overlay、git worktree / branch isolation 是多 subtask / 多 agent 场景中值得比较的三类 workspace sharing model。 | Inferred | Turso AgentFS repo / docs; IBM `chuk-artifacts`; IBM `chuk-mcp-runtime`; `workspace-structure.md` | 作为工作性分类，不写成行业标准。 |
| Overlay / copy-on-write 更适合作为共享与隔离之间的实现路径，不应等同于完整 checkpoint / snapshot。 | Inferred | Turso AgentFS repo / docs; `workspace-checkpoint.md`; `rollback-recovery-design-paths.md` | 需要继续补充具体实现的并发语义与非文件状态边界。 |
| `pause`、`interrupt`、`resume` 的差异本身会改变 lifecycle 设计语义：有的系统支持“等待当前动作结束后暂停”，有的则支持“立即取消执行再恢复”。 | Verified | `../../06-frameworks-and-tools/03-project-studies/openhands/runtime-and-sandbox.md` | 不应把所有“暂停恢复”都视为同一种 workspace lifecycle 能力。 |
| OpenHands 的调度风险图显示，workspace lifecycle 的启动 / 恢复成本可能来自 sandbox 轮询、workspace bridge、event persistence、setup scripts 与清理回收等多层编排，而不是单一 runtime 或语言性能。 | Observed / Inferred | `../../06-frameworks-and-tools/03-project-studies/openhands/scheduling-performance.md` | 可作为环境层通用提醒，但实际影响仍需压测验证。 |
