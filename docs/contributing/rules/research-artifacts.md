...
### 3.1.1 持续演进对象不要压成单一静态总览

如果研究对象具有明显版本演进特征，且不同版本差异会改变能力边界、接口行为、实现判断、发布形态或研究结论，不要把多版本观察压成单一静态对象总览。

更稳的组织方式通常是：

- 目录级 `overview.md`：承接系列总览、版本关系与研究边界
- 带版本标识的对象正文：承接具体版本对象研究

命名时：

- `overview.md` 只保留给目录级总览，不作为具体版本对象正文的默认后缀
- 具体版本对象正文优先使用带版本标识的对象文件名，如 `glm-5.2.md`
- 当文档承接的是版本对象本身时，即使引用了上游 report 作为来源，仍使用版本对象文件名
- 只有当文档主对象就是明确存在的上游 report / technical report / system card / whitepaper 时，才优先使用 `*-report.md`

不要因为“版本对象文也承担快速理解功能”，就把它写成 `*-overview.md`；那会把目录级元信息文件和版本对象正文重新混在一起。
