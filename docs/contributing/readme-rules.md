...
README 不再默认承担目录结构展示；目录结构与查找导航默认由 `index.md` 承接。

默认判断基线是：目录下已有两个及以上语义稳定、预期会持续保留的子目录时，就应优先考虑创建一个骨架型 `index.md`；不要先让 README 代行目录树或索引职责。只有在当前层级尚未达到这条基线，或明确属于不启用 `index.md` 的少数例外时，才按实际需要保留极小的过渡性结构说明。
...
不要把 README 里的目录树当作 `index.md` 的默认替代。目录下已有两个及以上语义稳定、预期会持续保留的子目录时，就应优先把结构展示迁到骨架型 `index.md`。README 里如保留极小的过渡性结构说明，也不应继续膨胀成常规目录树或索引页。
...
如需目录结构展示、同目录导航、按问题找入口、按阅读目标找入口，优先使用 `index.md` 承接，而不是继续扩写 README。`index.md` 应比现在更早出现：默认在目录下已有两个及以上语义稳定、预期会持续保留的子目录时，就可先创建一个骨架型 `index.md`，至少包含目录结构和直接子目录的一句话定位；只有查找压力继续上升时，才再展开成按问题找入口、按阅读目标找入口等更细的导航表。
