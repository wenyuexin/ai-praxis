...
- 日常任务记录
- 临时决定、一次性批次说明
- 还没有形成稳定问题模式的零散感想（单次问题轨迹记录放 [`incidents/`](../incidents/README.md)）
...
## 与主规则、`intent/`、`incidents/` 的关系

- 主规则负责：路由、职责边界、stop-line、默认执行顺序
- `intent/` 负责：解释规则背后的设计原理、原始意图与概念边界，防止对条文本身产生抽象误读
- 本目录负责：已确认复发模式的边界案例、误判复盘、对象级样本
- `incidents/` 负责：单次治理问题的轨迹记录，支持复发识别和规则沉淀

因此，主规则应保持短；`intent/` 解释“为什么这样定”；本目录说明“复杂场景里具体怎么用”；`incidents/` 记录“这个问题出现过，下次可以对照”。
