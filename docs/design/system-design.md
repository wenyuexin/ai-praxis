...
这四类目标之间不是并列清单：积累知识是目的，方法论是保障，知识沉淀又是方法论的实践与验证——它们是一对协同进化的过程。这套动态的协同进化模型见 [`evolution-design.md`](./evolution-design.md)；本文其余部分聚焦系统的静态分层与组成。
...
回答：人类读者或 AI 如何从问题出发，在树结构仓库中快速找到最可能的入口或目标位置；必要时先用 `README.md` / `index.md` 缩小范围，再在候选支路内做定向搜索。
...
- 如何利用 `index.md` 做层级下钻
- 什么时候该先沿目录树缩小范围，什么时候该在候选范围内做定向搜索
