...
`docs/` 下的 `README.md`、`index.md` 与其他元信息文件沿用仓库中的相同基本语义，但不要求各子目录使用与知识正文目录相同的文件组合。

判断文件是否应出现时，先看所在层的职责：

- 知识正文目录按主题理解、研究推进与材料组织需要决定元信息文件。
- `docs/contributing/` 优先承接可执行规则、设计意图、案例复盘与问题轨迹。
- `docs/capabilities/` 优先承接能力说明与能力分流。
- `docs/design/` 优先承接系统设计与层间关系说明。

不要因为知识正文目录存在 `overview.md`、`backlog.md`、`candidates.md` 或 `roadmap.md`，就机械地在 `docs/` 下创建同名文件。`docs/` 子目录是否需要 `README.md` 或 `index.md`，仍取决于是否存在真实的目录说明或查找导航问题，而不是为了结构对称而补齐。
