...
- 保留 `GLM-5.1` 属于超大规模 MoE 模型、激活参数约 `40B` 这一层
- 当前公开口径已更明显地收敛到 `754B / 40B`
- 但正文里仍不宜把总参数写成完全无条件的单一确定数字；更稳的写法是把 `754B / 40B` 视为当前主公开口径，同时保留后续以更直接 config / model card 继续核实的空间
